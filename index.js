...
function createBot() {
  ...
  try {
    ...
    // "auto" (or an empty string) means "let mineflayer negotiate the protocol
    // itself" - it must become the boolean `false`, never the literal string
    // "auto"/"false", or mineflayer will try to parse it as a real MC version
    // and fail outright.
    const botVersion = selectedVersion && selectedVersion.trim() !== '' && selectedVersion !== 'auto' ? selectedVersion : false;
