...
function start(bot, mcData, config, addInterval) {
  ...
  addInterval(async () => {
    ...
    try {
      ...
      if (decisionCfg.thinkDelay === true) {
        const min = decisionCfg.thinkMinMs == null ? 150 : decisionCfg.thinkMinMs;
        const max = decisionCfg.thinkMaxMs == null ? 400 : decisionCfg.thinkMaxMs;
  ...
  }, Math.max(1500, cfg.interval || 4000));
}
