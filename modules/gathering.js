...
async function huntNearbyAnimal(bot) {
  ...
  const entity=Object.values(bot.entities||{})
    .filter(e=>e.name&&ANIMALS.has(e.name)&&e.position)
    .sort((a,b)=>bot.entity.position.distanceTo(a.position)-bot.entity.position.distanceTo(b.position))[0];
  if(!entity) return false;
  try {
    if(!(await gotoPos(bot,entity.position,2.8,9000))) return false;
    ...
    while(Date.now()<deadline && bot.entities[entity.id]) {
      if (bot.entity.position.distanceTo(entity.position) > 3.8) {
        await gotoPos(bot,entity.position,2.8,2500);
      }
      if (bot.entity.position.distanceTo(entity.position) <= 4.0) bot.attack(entity);
      await sleep(420);
    }
    return true;
