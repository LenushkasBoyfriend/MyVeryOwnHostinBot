...
const { log, gotoBlock, findOneBlock, countItem, sleep, safePlace } = require('./utils');
...
async function ensureCraftingTable(bot, mcData) {
  ...
  const placed = await safePlace(bot, 'crafting_table', refBlock, require('./utils').Vec3(0, 1, 0));
...
async function ensureFurnace(bot, mcData) {
  ...
  const placed = await safePlace(bot, 'furnace', refBlock, require('./utils').Vec3(0, 1, 0));
  if (!placed) return null;
  await sleep(300);
