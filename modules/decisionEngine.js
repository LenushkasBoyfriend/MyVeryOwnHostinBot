...
function scoreGoals(bot, cfg, state) {
  ...
  if (logs < 8) add('wood-starter', 110, 'Starter wood is missing.', 'wood');
  else if (!s.hasPickaxe) add('tools-starter', 108, 'A pickaxe is required for progression.', 'tools');
  else if (s.foodItems < 4) add('food-starter', 96, 'A small food reserve is required.', 'food');
  ...
  if ((state.knowledge?.lastResearchAt || 0) + 45 * 60 * 1000 < Date.now()) {
    add('research', 2, 'Background knowledge maintenance.', 'research');
  }
  add('explore', 8, 'No urgent survival task dominates.', 'explore');
